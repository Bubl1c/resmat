...
class CrossSectionSolver(input: GeometryShapesProblemVariantInputData) {
...
//  }
//
//  def apply(confs: Seq[ProblemInputVariableConf], vals: Seq[ProblemInputVariableValue]): GeometryShapesProblemInputData = {
...
case class ShapeCalculatedData(
  ...
  override def toString: String = {
    s"""Shape Geometry Characteristics $id $shapeName
      |square   = $square
      |I_y      = $I_y
      |I_z      = $I_z
      |I_yz     = $I_yz
